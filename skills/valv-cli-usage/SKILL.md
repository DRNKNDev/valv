...
| `sync_timed_out` (exit `75`) | `valv sync` didn't settle inside its bound. Retry; if it keeps happening, run `valv status` to see which mount has pending ops piling up. |
| `sync_mount_error` (exit `1`) | A mount entered a persistent error state (not a transient per-pass error, which `sync` retries instead). Run `valv status` for the mount's own error detail. |
