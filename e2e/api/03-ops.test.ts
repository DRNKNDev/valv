...
describe("op submission API", () => {
  ...
  it("applies all op types, enforces CAS, and rejects collisions", async () => {
    ...
    expect(row<{ name: string }>(ctx.sqlite, "SELECT name FROM nodes WHERE node_id = ?", file.nodeId)?.name).toBe("renamed.txt");
    ...
    expect(row<{ parent_id: string }>(ctx.sqlite, "SELECT parent_id FROM nodes WHERE node_id = ?", file.nodeId)?.parent_id).toBe(folder.nodeId);
    ...
    expect(row<{ deleted_at: number }>(ctx.sqlite, "SELECT deleted_at FROM nodes WHERE node_id = ?", file.nodeId)?.deleted_at).toBeTruthy();
