...
import { createNode, requestJson, submitOp, uploadChunk } from "./helpers.js";
import type { SeededHarness } from "./types.js";

export function blobstoreScenarios(harness: SeededHarness): void {
  describe("blobstore API", () => {
    ...
    it("coordinates chunk upload, dedupes referenced chunks, and accepts uploaded versions", async () => {
      ...
      expect((await ctx.row<{ refcount: number }>("SELECT refcount FROM chunks WHERE chunk_hash = ?", hash))?.refcount).toBe(1);
