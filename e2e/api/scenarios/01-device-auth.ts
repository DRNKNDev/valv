...
import type { BareHarness } from "./types.js";

export function deviceAuthScenarios(harness: BareHarness): void {
  describe("device auth API", () => {
    let setup: Awaited<ReturnType<BareHarness["createApp"]>>;

    beforeAll(async () => {
      setup = await harness.createApp();
    });
