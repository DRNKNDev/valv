  ...
  try {
    ...
    for (const chunk of chunks) {
      ...
      try {
        ...
        await db.execute?.(sql`DELETE FROM chunks WHERE chunk_hash = ${chunkHash} AND refcount = 0`);
      } catch (error) {
...
async function runTombstonePurge(db: CoreDb, retentionMs: number): Promise<void> {
  try {
    ...
    await db.execute?.(sql`DELETE FROM nodes WHERE deleted_at IS NOT NULL AND deleted_at < ${cutoff}`);
  } catch (error) {
...
async function runOpLogTruncation(db: CoreDb, retentionMs: number): Promise<void> {
  try {
    const cutoff = new Date(Date.now() - retentionMs);
    await db.execute?.(sql`DELETE FROM op_log WHERE applied_at < ${cutoff}`);
  } catch (error) {
